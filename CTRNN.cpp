...
void CTRNN::SetCircuitSize(int newsize)
{
  ...
  parameters.SetBounds(1, size*size+2*size);
  parameters.FillContents(0.0);
	...
	k4.SetBounds(1,size);
  //NEW FOR RL
  weightcenters.SetBounds(1,size,1,size);
  weightcenters.FillContents(0.0);
  biascenters.SetBounds(1,size);
	biascenters.FillContents(0.0);
  learnrate = 0.008;
  initamp = 0.00001;
  amp=initamp;
  convergence = 0.004;
  ...
  fluxPeriodMin = 3000;
  fluxPeriodMax = 4000;
  //meanPeriod = 3500; // in steps
  //
  //stdPeriod = 2500;  // in steps
  fluxsize = int(size*size + size);
  flux.SetBounds(1,fluxsize);
  flux.FillContents(0.0);
  period.SetBounds(1,fluxsize);
  period.FillContents(0);
  periodcount.SetBounds(1,fluxsize);
  for (int i = 1; i <= fluxsize; i++)
  {
    period[i] = int(UniformRandom(fluxPeriodMin, fluxPeriodMax));
    //period[i] = period[i] < int(meanPeriod/2) ? int(meanPeriod/2) : period[i];
  }
}
...
void CTRNN::Flux(RandomState &rs)
{
  // Update periods
  for (int i = 1; i <= fluxsize; i++)
  {
      if (periodcount[i]==0) // Generate new period from distribution
      {
        period[i] = int(rs.GaussianRandom(meanPeriod,stdPeriod));
        period[i] = period[i] < int(meanPeriod/2) ? int(meanPeriod/2) : period[i];
        periodcount[i] = period[i];
        flux[i] = 0.0;
      }
      else
      {
        flux[i] = sin(2*Pi*(float(periodcount[i])/float(period[i])));
        periodcount[i]--;
      }
  }

  // Update the effective parameters of all the neurons based on random fluctuations
  int k = 1;
  for (int i = 1; i <= size; i++) {
    for (int j = 1; j <= size; j++) {
      weights[j][i] = weightcenters[j][i] + amp * flux[k++];
    }
    biases[i] = biascenters[i] + amp * flux[k++];
  }
...
void CTRNN::Learn(double performance)
{

  reward = performance-runningaverage.avg(); //< 0 ? 0.0 : performance-pastperf;
  amp-= convergence*reward;
  amp = std::min(std::max(amp, 0.0), max_flux_amp);
  for (int i = 1; i <= size; i++) {
    for (int j = 1; j <= size; j++) {
      weightcenters[j][i] += learnrate * (weights[j][i] - weightcenters[j][i]) * reward;
      weightcenters[j][i] = clip(weightcenters[j][i], -weightRange, weightRange);
    }
    biascenters[i] += learnrate * (biases[i] - biascenters[i]) * reward;
    biascenters[i] = clip(biascenters[i], -biasRange, biasRange);
  }

  //runningaverage.push(performance);
}
...
void CTRNN::recoverParameters(){
    ...
    double m = (getRange(timeRange[0], timeRange[1]))/2;
    double b = timeRange[0] - m*(-1);
    //timeconstants
    for(int i =1; i<=size;i++){
      parameters[i] = (NeuronTimeConstant(i)-b)/m;
    }
    //biases
    m = (getRange(-biasRange, biasRange))/2;
    b = -biasRange - m*(-1);
    for(int i =1; i<=size;i++){
      parameters[size+i] = (NeuronBias(i)-b)/m;
    }
    //weights
    m = (getRange(-weightRange, weightRange))/2;
    b = -weightRange -m*(-1);
    for(int i =1; i<=size;i++){
      for(int j =1; j<=size;j++){
      int index = 2*size+1+(i-1)*size+(j-1);

      parameters[index] = (weightcenters[i][j]-b)/m;
      }
    }
}
